...
- **Phase**: nb04 ceiling probe RUN on Colab (2026-06-06). TimesFM pivot REFUTED. RAW TimesFM-2.0-500m (close channel) decodes future_volatility at val R² **−0.134** — BELOW the untrained-random floor (+0.097) and far below a trivial 4-moment baseline (**+0.333**), with severe overfit (train +0.246, α pinned at grid max). Decisive surprise: a 24-d `[mean,std,last,sum]`-per-channel baseline beats EVERY learned/pretrained representation on volatility. Return ≈ unpredictable (all ≤ +0.012); direction signal weak and only the random projection (0.571) captures it. → HOLD the pivot; do NOT spend training units on a frozen-TimesFM swap.
- **Primary metric**: Downstream linear probe. val_jepa is secondary and must always be reported alongside its shuffled-target control ratio. NEW BAR for volatility: the +0.33 raw-moment baseline, NOT the +0.097 random floor — any learned model must beat the trivial baseline to justify training.
- **Open decision**: confirm what carries raw-input's +0.333 (forward passes only): probe raw-input on UN-normalized context + a single feature = raw context std, to test the "per-sample normalization removes the volatility-level signal" hypothesis. If raw σ alone reproduces ~+0.33, the INPUT PIPELINE (not the backbone) is the lever. Only if inconclusive, run TimesFM `variant="concat6"` (all 6 channels) to close the channel confound.
- **Decided**: Drop VICReg invariance term. Prefer Qwen2.5-1.5B + LoRA as predictor (Option B). Price encoder default = `transformer` backend. TimesFM loads via the HF `transformers` integration (`TimesFmModelForPrediction`), NOT the `timesfm` pip package (numpy<2 risk). HOLD on the TimesFM pivot — the ceiling probe did not clear the floor. No backbone-swap training until the raw-input/normalization question is resolved.

### Log

**2026-06-06 (run) -- nb04 ceiling probe RESULT: TimesFM REFUTED, raw 4-moment baseline wins (research_log/entries/2026-06-06_timesfm-ceiling-probe.html)**
- Colab run, n_train=100 batches / n_val=50 / seed=42 / wide α grid 0.01..1e4. future_volatility val R²: raw-input **+0.333**, untrained z_price (FLOOR) +0.097, trained z_price +0.118, **TimesFM (CEILING) −0.134**. future_return val R²: all ≤ +0.012 (raw +0.012, untrained −0.041, trained −0.058, timesfm −0.081). direction val acc: untrained **0.571** (only one >majority+2·SE), raw 0.528, trained 0.535, timesfm 0.539 (majority 0.528).
- **TimesFM does NOT clear the ceiling**: −0.134 is BELOW the random floor (+0.097) and far below raw inputs (+0.333), with severe overfit (vol train +0.246 → val −0.134) and α pinned at grid max (vol/return) / grid min (direction) — D=1280 straining the probe at both ends; widening won't rescue a maximally-regularized −0.134. → HOLD the pivot; no frozen-TimesFM training.
- **Dominant finding**: a 24-d `[mean,std,last,sum]`-per-channel baseline beats every learned/pretrained representation on volatility (+0.333, val ≥ train, α=1000 mid-grid → robust). The volatility signal is real, strong, trivially linear. Leading hypothesis for why the encoders can't match it: the causal per-window scale normalization removes the absolute volatility level (the best predictor of future vol); raw-input likely recovers a residue via the `σ+1e-6` floor + "last" values. → NEXT (forward passes only): probe raw-input on UN-normalized context + a single raw-σ feature to test this; the input pipeline (not the backbone) may be the lever for the vol target.
- **Caveats**: TimesFM saw only the close channel (`variant="close"`); raw-input saw all 6 → channel confound (concat6 untested, deprioritized). nb04 verdict-string had a bug (printed "INTERMEDIATE: beats floor" when C<Fl); fixed to emit "REFUTE/HOLD: TimesFM is BELOW the random floor" for C<Fl−0.01. Numbers were always correct; only the label was wrong.
- **Re-bar**: for volatility the reference is now +0.33 (raw moments), NOT the +0.097 random floor. Any learned/JEPA model must beat the trivial baseline to justify training units.
